...
// Run starts the comms mini service.
func Run() {
	app, cfg := svcutil.BootstrapWithConfig("comms", Version, "Channels and messages", config.FromEnv)
	defer app.Shutdown()
	...
	slog.Info("comms service started",
		"data", app.DataDir,
		"version", app.Version,
	)
	app.Wait()
