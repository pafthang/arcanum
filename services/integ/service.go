...
import (
	...
	"github.com/pafthang/arcanum/services/integ/internal/apis"
...
// Run starts the integ mini service.
func Run() {
	app, cfg := svcutil.BootstrapWithConfig("integ", Version, "External connectors", config.FromEnv)
	defer app.Shutdown()
	...
	slog.Info("integ service started",
		"data", app.DataDir,
		"version", app.Version,
	)
	app.Wait()
